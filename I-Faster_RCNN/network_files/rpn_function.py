...
class AnchorsGenerator(nn.Module):
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
        ...
        也就是将anchors模板映射到原图中，并获取在原图中的anchors信息
        """
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 根据提供的sizes和aspect_ratios生成anchors模板----base_anchors
...
class RPNHead(nn.Module):
    ...
    通过滑动窗口计算预测目标概率与bbox regression参数（得到预测目标概率分数和边界框回归参数也就是边界框的坐标偏移量）
    ...
    Arguments:
        ...
        该函数的第一个输出logits的构成：（该参数是按预测特征层划分的，resnet有4层，mobilnet只有一层）
        该参数是一个四维向量，[一个batch中图片的数量，需要预测的anchors的个数，特征层的hight，特征层的wight]
        4维矩阵中的每个数就是该pixel上对应的15个anchors的预测概率值
        注意：这里的预测参数只预测该anchors内的区域是前景还是背景
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        """
        box_cls_flattened和box_regression_flattened是用来装改变shape后的变量
        box_cls和box_regression中的信息是按照预测特征层划分的
        现在这个函数的作用就是将每个预测特征层单独取出来，改变shape，因为每个预测特征层大小不一，所以需要遍历操作
        最后所得到的变量仍然是按照每个预测特征层划分的
        
        permute_and_flatten函数所进行的操作：
        （2，15，48，84） -> （2，15，1，48，84） -> （2，48，84，15，1） -> （2，60480，1）
        
        最后输出的box_cls和box_regression：
        box_cls和box_regression是调整shape后的预测框边界信息和预测框回归信息
        
        concat_box_prediction_layers这个函数的功能：
        就是将原本的（2，15，48，84）和（2，60，48，84）变为了（12960，1）和（12960，4）
        原本的输入是一个list，该list的每个元素是按照预测特征层划分的，处理后的变量为一个整合的list，最后展平为一个tensor，
        就是把原本每个预测特征层中的信息经过reshape后append到了该list中，该list中的信息就不再是按照预测特征层单独划分的，该list中
        只有一个元素，该元素中存储着所有的预测特征层中的信息，list（0;1;2;3） -> list(0)
        最后再经过torch.cat变为tensor格式
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                # 计算每一个anchors与每一个真实bbox之间的的iou信息，所以match_quality_matrix维度为（num-bbox，num-anchors）
                # set to self.box_similarity when https://github.com/pytorch/pytorch/issues/27495 lands
                match_quality_matrix = box_ops.box_iou(gt_boxes, anchors_per_image)
                # 计算每个anchors与gt匹配iou最大的索引（如果iou<0.3索引置为-1，0.3<iou<0.7索引为-2）
                # matched_idxs存储的是每个anchors对应最大iou的gt的索引，存的是gt的索引
                ...
                # 上述6行代码，实现了将正样本设为1，负样本0，丢弃的设为-1

            # lables记录的是每一个anchors是正样本负样本还是丢弃的
            # matched_gt_boxes记录的是每一个anchors匹配到的gt-box的坐标
            # 一个记录是正样本还是负样本，另一个记录对应的具体坐标
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        """
        levels内储存的是索引值，就是说不同特征层的anchors索引值不同，都放在levels中存储
        
        这里levels的作用是：
        首先现在只是知道每个预测特征层上预测anchors的数目（num_anchors_per_level）
        并不知道每个anchors具体在哪个预测特征层
        最终是为了给proposals一个对照，知道proposals分别是对应的哪一个预测特征层
        """
        ...
        """
        根据objectness中的预测概率，遍历每个预测特征层，获取每个预测特征图上预测概率排前pre_nms_top_n的anchors索引值
        后续还需要用top_n_idx来切片出anchors的索引和概率objectness的索引
        """
        ...
        #上面这3行切片语法，暂时没看懂，不管了，知道它是根据top_n_idx进行切片，得到前多少多少个元素
    ...
    def forward(self,
        ...
        Arguments:
            ...
            targets (List[Dict[Tensor]): ground-truth boxes present in the image (optional).
                If provided, each element（元素） in the dict should contain a field `boxes`,
        ...
        """
        anchor_generator的结果是一个list，每个元素为一张图像中的所有anchors信息，
        生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        list中每个元素的内容是由每一张图像的所有预测特征层的anchors坐标信息拼接在一起所得到的
        list(0,1) -> 0:（60480，4）, 1:（60480，4）
        """
        ...
        """
        这里就是套用论文中的公式，就是把利用anchors_generator生成的anchors和预测的边界框回归参数结合起来
        相当于更新了预测边界框
        proposals维度为(120960,1,4)
        """
